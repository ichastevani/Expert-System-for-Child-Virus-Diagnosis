...
class Inference:
    ...
    def __askQuestion(self):
        ...
        for clause in self.__clauseBase:
            print()
            global userInput 
            userInput = str(Inference.Result).strip()
            output = self.__inferenceResolve(userInput)
            Inference.Virus = output
            try:
                if Inference.Virus[0]:
                    Log.i(clause.getPositive() + Inference.Virus[1])
                else:
                    Log.i(clause.getNegative() + Inference.Virus[1])
            except TypeError:
                print("Kemungkinan kamu tidak terkena 3 virus tersebut. Silahkan cek ke dokter :)")

        Log.i("Aku harap kamu puas dengan prediksinya :)")
    ...
    def __runForwardChain(self, userBase):
        ...
        for target, percent in matchesRules.items():
            if percent >= PERCENT_MATCH:
                return True, target + " dengan persentase " + str(percent) + " %"
            elif percent == 0:
                return False, target
            else:
                return False, target + " dengan persentase " + str(percent) + " %"
